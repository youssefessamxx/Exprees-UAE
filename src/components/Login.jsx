...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        Cookies.set("authToken", access, { httpOnly: true, secure: true });
        Cookies.set("refreshToken", refresh, { httpOnly: true, secure: true });
  ...
  // if (isLoggedIn) {
  //   return (
  //     <div className="text-center py-[100px] px-8 md:px-12">
  //       <Toaster position="top-center" reverseOrder={false} />
  //       <h2 className="font-bold text-[32px] mb-4 text-white">
  //         You are already logged in
  //       </h2>
  //       <Link className="font-bold text-[#F05B1F]" to="/">
  //         Go to Home
  //       </Link>
  //     </div>
  //   );
